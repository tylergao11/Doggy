...
//! `off`. Custom profiles via `~/.Doggy/sandbox.toml` or `.Doggy/sandbox.toml`.
...
/// Load sandbox config from `~/.Doggy/sandbox.toml` and `.Doggy/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.Doggy/sandbox.toml
    let global_path = grok_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.Doggy/sandbox.toml (additive only)
    let project_path = workspace.join(".Doggy").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    ...
    let project =
        load_config_file(&workspace.join(".Doggy").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // directories like ~/.Doggy/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.Doggy/sandbox.toml or .Doggy/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
