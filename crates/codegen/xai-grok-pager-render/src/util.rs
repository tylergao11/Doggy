...
/// User-facing label for the user grok directory (``~/.Doggy`` or ``$GROK_HOME``).
///
...
/// User-facing path under [`grok_home()`], e.g. ``~/.Doggy/config.toml``.
...
mod tests {
    ...
    fn display_grok_home_prefix_default_install() {
        ...
        assert_eq!(display_grok_home_prefix(), "~/.Doggy");
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.Doggy/memory/MEMORY.md");
