// claude_import.rs
// Scans Claude settings and generates TOML patches for .Doggy/config.toml.
//
...
pub enum ImportScope {
    /// User-level: writes to `~/.Doggy/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.Doggy/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.Doggy/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.Doggy/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.Doggy/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.Doggy/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.Doggy/config.toml`.
///
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.Doggy/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.Doggy/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        ...
        // Hooks are written separately to ~/.Doggy/hooks/imported-from-claude.json.
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".Doggy").join("config.toml");
        ...
        let hooks_dir = project_root.join(".Doggy").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .Doggy/hooks/ JSON files in apply_hooks_to_dir,
...
/// (Claude-compatible). The native hooks loader scans `.Doggy/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .Doggy/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.Doggy) don't interfere.
...
"#;
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.Doggy/config.toml` may legitimately have the marker
