...
pub(crate) mod goal_criterion_graph;
pub(crate) mod goal_deterministic_gate;
pub(crate) mod goal_fanout;
...
/// Doggy task completion authority (Round vs Task). See `docs/编排/完成权.md`.
/// Pure policy lives in `xai-doggy-orchestrator` so it can be tested without host deps.
pub use xai_doggy_orchestrator as orchestrator;
pub mod helpers;
