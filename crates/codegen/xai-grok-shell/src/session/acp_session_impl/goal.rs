...
impl SessionActor {
    ...
    pub(super) async fn drain_goal_updates_with_extra(
        ...
        for source in cmds {
            ...
            // Record Achieved verdict only; TaskOrchestrator marks Complete
            // after audit pass (`doggy_mark_task_done`).
            if !policy.enabled {
                let (tokens_used, finished_marginal) = self.goal_tokens(current_tokens);
                ...
                // Goal stays Active until doggy TaskDone.
                notify.emit_goal_updated(&mut tracker, tokens_used, finished_marginal);
                drop(tracker);
    ...
    async fn apply_classifier_outcome(
        ...
        match outcome {
            GoalClassifierOutcome::Achieved { details_path } => {
                // Doggy: verification pass is an *audit input*, not TaskDone.
                ...
                let details_path = {
                    ...
                    // NotAchieved streak is broken; goal stays Active for
                ...
                // Summarizer runs only after doggy TaskDone (see doggy_gate).
            ...
            GoalClassifierOutcome::FailOpenAchieved {
                ...
                // Doggy root-cause fix: fail-open must NEVER complete the task.
                // Record NotAchieved so audit fails and the implementer continues
                // (or infra is retried), rather than fake Done.
                tracing::warn!(
                    ?reason,
                    "goal verification fail-open (infra-class) → Doggy refuses Done \
                     (recorded as NotAchieved)"
                );
                self.clear_pending_classifier_completions();
                let mut tracker = self.goal_tracker.lock();
                let fail_msg = format!(
                    "Verification infrastructure failed ({}); task is NOT complete. \
                     Retry verification after fixing environment issues.",
                    reason.as_const_str()
                );
                Self::record_verdict_on_orchestration(
                    &mut tracker,
    ...
    /// Inject a continuation / fix directive into the conversation (Doggy
    /// single inject path). Prunes prior goal-continuation directives so
    /// only the latest copy stays in history.
