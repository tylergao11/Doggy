...
impl SessionActor {
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let result = {
            ...
            loop {
                ...
                // Free chat (no Active goal): one successful round ends the prompt.
                // Task-bound sessions: Doggy orchestrator owns Continue / Audit / Done.
                if !self.doggy_task_bound() {
                    break round;
                }
                if !matches!(
                    doggy_machine.status,
                    xai_doggy_orchestrator::TaskStatus::Active
                ) {
                    doggy_machine.start();
                }
    ...
    /// after every turn finishes.
    ///
    /// **Doggy (Phase D):** successful-turn continuation is owned **only**
    /// by the in-turn [`Self::run_doggy_round_end`] loop inside
    /// `handle_prompt`. This method must **not** call
    /// `maybe_queue_goal_continuation` on success — that legacy queue was a
    /// second completion path and reintroduced premature-done / double-nudge
    /// races. Success only resets `goal_continuation_streak`.
    ///
    /// **Non-success** (cancel / infra / max-turns): still enforce token
    /// budget → `BudgetLimited`, then increment the failed-turn streak and
    /// auto-pause at [`GOAL_CONTINUATION_BACKOFF_THRESHOLD`]
    /// (`GoalPauseReason::BackOff`). That is a safety valve, not TaskDone.
    ///
    /// When the goal is not `Active`, both branches are skipped.
    pub(crate) async fn handle_turn_end(&self, turn_succeeded: bool) {
        ...
        if turn_succeeded && goal_active_now {
            ...
            // Deliberately NO maybe_queue_goal_continuation — Doggy in-turn.
            if self.goal_tracker.lock().status()
                == Some(crate::session::goal_tracker::GoalStatus::Active)
            {
                tracing::debug!(
                    session_id = %self.session_info.id.0,
                    "doggy: turn succeeded with goal still Active; \
                     continuation is in-turn only (no legacy GoalSummary queue)"
                );
            }
            return;
        }
