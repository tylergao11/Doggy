...
//! On a hit (when the **legacy** `maybe_queue_goal_continuation` /
//! `prepare_goal_continuation` path runs), the bail-specific
//! continuation nudge is selected and the harness emits
//! `Event::GoalPrematureStopDetected { pattern }`.
//!
//! **Doggy:** production completion no longer uses this detector to
//! decide TaskDone — only the orchestrator + audit gate may complete a
//! task. The detector remains for telemetry and legacy/test queue paths.
//!
