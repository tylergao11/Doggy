...
/// A feedback entry persisted to `~/.Doggy/sessions/.../feedback.jsonl`.
///
...
/// Searches across ALL cwd directories under `~/.Doggy/sessions/`.
///
/// Use `session_exists_for_cwd` instead when the target cwd is known
/// (e.g., the `-r` resume path) to avoid false-positive matches.
/// Find a session by ID across **all** CWD directories under `~/.Doggy/sessions/`.
///
...
/// Collect MCP server stderr logs from `~/.Doggy/logs/mcp/` for inclusion in the session archive.
...
/// Walk `~/.Doggy/sessions/` and delete files with mtime older than `ttl_days`.
