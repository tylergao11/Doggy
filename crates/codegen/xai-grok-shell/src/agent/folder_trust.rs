...
//! inside a cloned repository (`.mcp.json`, project `.Doggy/config.toml`,
...
// NOTE: this folder-trust store (`~/.Doggy/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`xai_grok_agent::plugins::TrustStore` at `~/.Doggy/trusted-plugins`, plus the
...
///
/// Sources: project `.Doggy/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.Doggy/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.Doggy/config.toml [mcp_servers]` entries tagged project (the loader's key
...
mod tests {
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        std::fs::create_dir_all(home.path().join(".Doggy").join("hooks")).unwrap();
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".Doggy").join("agents");
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.Doggy/hooks`), and no store grant must be
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.Doggy/plugins/<x>/`, no
        ...
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".Doggy").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".Doggy").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        let plugin = tmp.path().join(".Doggy").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".Doggy").join("hooks");
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        ...
        // A `<cwd>/.Doggy/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.Doggy/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.Doggy/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let grok = tmp.path().join(".Doggy");
        std::fs::create_dir_all(&grok).unwrap();
        std::fs::write(grok.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// (`.mcp.json`) and `projtoml` (`.Doggy/config.toml [mcp_servers]`).
    ...
    /// `.Doggy/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".Doggy").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".Doggy").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".Doggy").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".Doggy").join("lsp.json"), "{}").unwrap();
