...
pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `grok login`, set XAI_API_KEY, or add api_key to ~/.Doggy/config.toml.";
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.Doggy/auth.json".to_string())),
    )
}
...
mod tests {
    ...
    /// `~/.Doggy/config.toml` skeleton from the bug report, walks it through
