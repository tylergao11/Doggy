...
/// Returns `~/.Doggy/worktrees/<repo_slug>` for the given git root.
///
...
/// Resolves the worktree base directory (`~/.Doggy/worktrees/<repo_name>`)
/// for a given source path, correctly handling grok-managed worktrees.
///
/// When `source_path` is already under `~/.Doggy/worktrees/<repo>/...`, the
...
/// Walk up from `cwd` (staying within `~/.Doggy/worktrees/`) to its registered
/// worktree record.
///
...
/// returns `None` (without DB I/O) for paths outside `~/.Doggy/worktrees/`.
...
///
/// When the source path is already inside `~/.Doggy/worktrees/<repo>/`, the
...
/// Scan `~/.Doggy/worktrees/<repo_name>/` for subdirectories not tracked
