...
//! `~/.Doggy/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.
...
/// `~/.Doggy/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.Doggy/memory/xai-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.Doggy/memory/`
    global_dir: PathBuf,
    /// `~/.Doggy/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.Doggy/memory/`.
    ///
    ...
    ///
    /// File path: `~/.Doggy/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
