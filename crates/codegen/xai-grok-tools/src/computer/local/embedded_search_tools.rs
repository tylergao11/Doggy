...
//! (release builds, self-extracted to `~/.Doggy/vendor/<name>-<ver>-<target>`) →
//! `~/.Doggy/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
// (release pipeline). Self-extracted to `~/.Doggy/vendor` on first use, mirroring
...
/// Write embedded `bytes` to `~/.Doggy/vendor/<versioned_name>` (chmod 755) on
...
/// `~/.Doggy/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
mod tests {
    ...
    /// bytes self-extract under `~/.Doggy/vendor` and the extracted `bfs` runs.
