...
///
/// `skills` is the standard layout (`.Doggy/skills/`, `.claude/skills/`,
...
/// name (e.g. `~/.Doggy/skills/shell`) is NOT dropped — only skills physically
...
/// `cwd` (exclusive). At each directory, checks for `.Doggy/skills/`,
...
pub fn discover_skills_for_paths(
    ...
    let mut config_dir_names: Vec<&str> = vec![".Doggy", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.Doggy is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.Doggy/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.Doggy/ → kept (user content).
        let grok_shell = tmp.path().join(".Doggy").join("skills").join("shell");
        ...
        assert!(skills[0].path.contains("/.Doggy/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        .unwrap();
        let grok_skill = sub.join(".Doggy").join("skills").join("grok-dyn");
