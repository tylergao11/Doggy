...
//!
//! Searches `.Doggy/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.Doggy/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.Doggy/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".Doggy/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.Doggy/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.Doggy/agents` / `.claude/agents`) under each
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.Doggy/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.Doggy/agents/` walking from `cwd` up to repo root
/// 2. `~/.Doggy/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.Doggy/bundled/agents/` (bundled, lowest priority)
///
...
/// (GROK_HOME-aware) plus the legacy literal `~/.Doggy` when GROK_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    grok_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.Doggy, included only when it differs from grok_home
...
///
/// Project-level `.Doggy/agents/` has highest priority, then falls back
...
/// Load project agent definitions from every `.Doggy/agents` / `.claude/agents`
...
mod tests {
    ...
    fn user_agent_dirs_dedups_legacy_when_grok_home_is_dot_grok() {
        ...
        assert_eq!(
            count, 1,
            "no duplicate ~/.Doggy/agents when grok_home == ~/.Doggy"
        );
    }
    ...
    fn test_discover_finds_md_files() {
        ...
        let agents_dir = tmp.path().join(".Doggy").join("agents");
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".Doggy").join("agents");
        let agents_dir_2 = inner_dir.join(".Doggy").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".Doggy").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".Doggy")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".Doggy")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".Doggy").join("agents");
        let bundled_dir = home.join(".Doggy").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".Doggy")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".Doggy")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".Doggy").join("agents");
        let bundled_dir = home.join(".Doggy").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .Doggy/agents/ directory — should fall back to built-in
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.Doggy/agents/) agent named "explore" should NOT shadow
