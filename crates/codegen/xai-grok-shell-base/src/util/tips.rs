...
//! persisted to `~/.Doggy/tip_cursor.json`.
...
/// Load the cursor from `~/.Doggy/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.Doggy/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.Doggy/tip_cursor.json`, so every session sees the next tip
