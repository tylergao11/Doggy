...
Grok stores data in `~/.Doggy` (override with `GROK_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.Doggy` read-only:
