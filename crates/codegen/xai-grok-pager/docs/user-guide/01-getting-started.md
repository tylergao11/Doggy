...
On first launch, Grok opens your browser to authenticate with grok.com. After you sign in, Grok stores your credentials in `~/.Doggy/auth.json`, where they persist across sessions. Grok refreshes your credentials automatically and prompts you to sign in again when they can no longer be renewed.
...
Every conversation is a **session**. Sessions are automatically saved to `~/.Doggy/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.
...
```
~/.Doggy/AGENTS.md           # Global rules (apply to all projects)
