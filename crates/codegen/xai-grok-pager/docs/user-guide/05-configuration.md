...
3. **config.toml** (`~/.Doggy/config.toml`)
...
`[ui] vim_mode` in `~/.Doggy/config.toml` immediately and applies it to every
...
MCP servers can also be configured per-project in `.grok/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.Doggy/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.grok/config.toml` (current dir) > `<repo-root>/.grok/config.toml` > `~/.Doggy/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).
...
`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.Doggy/config.toml`.
...
| Variable | Description |
|----------|-------------|
| `GROK_HOME` | Override config directory (default: `~/.Doggy`) |
...
| Path | Description |
|------|-------------|
| `~/.Doggy/config.toml` | Main configuration file |
| `~/.Doggy/pager.toml` | TUI appearance configuration |
| `~/.Doggy/auth.json` | Authentication credentials (auto-managed) |
| `~/.Doggy/sessions/` | Persisted sessions (organized by working directory) |
| `~/.Doggy/memory/` | Cross-session memory files and index |
| `~/.Doggy/skills/` | User-scoped skill definitions |
| `~/.Doggy/plugins/` | User-scoped plugins |
| `~/.Doggy/agents/` | User-scoped agent definitions |
| `~/.Doggy/lsp.json` | LSP server configuration (user-scoped) |
| `~/.Doggy/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
...
|------|--------------------|
| `.grok/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.Doggy/config.toml`) |
...
|--------|----------|-------|
| User | `~/.Doggy/lsp.json` | All projects |
