...
MCP servers are configured in `~/.Doggy/config.toml` under `[mcp_servers.<name>]` sections.
...
> - `config.toml` — user-level (`~/.Doggy/config.toml`) **or repo-level**
...
|----------|-------|----------|
| `~/.Doggy/config.toml` | All projects | Lowest |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Grok reads most other config sections only from `~/.Doggy/config.toml`.
...
|--------|--------|----------|-------------|
| `config.toml` | Native Grok config | `~/.Doggy/config.toml`, `.grok/config.toml` | Always on |
...
For stdio servers, Grok captures the process's standard error to `~/.Doggy/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.Doggy/logs/mcp/filesystem.stderr.log
```
