...
| `~/.Doggy/plugins/` | User -- personal plugins for every project | Trusted automatically |
...
Configure plugin directories and per-plugin state in `~/.Doggy/config.toml`:
...
To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.Doggy/pager.toml`:
...
Place this file at `~/.Doggy/settings.json` or `~/.claude/settings.json`.
...
Grok trusts plugins from `~/.Doggy/plugins/` automatically. Project plugins in `.grok/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:
