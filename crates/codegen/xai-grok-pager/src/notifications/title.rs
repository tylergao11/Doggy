...
/// User-visible product name in the terminal tab / window title.
/// Keep this the single source of truth — do not scatter "grok"/"doggy" literals.
...
mod tests {
    ...
    fn grok_only_produces_just_grok() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Doggy");
    }
    ...
    fn session_name_and_grok_joined_with_separator() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "my project - Doggy");
    }
    ...
    fn spinner_only_shown_when_active() {
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - Doggy"));
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - Doggy");
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - Doggy");
    }
    ...
    fn model_item_shown_when_present() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "grok-3 - Doggy");
    }
    ...
    fn turn_timer_shown_when_above_one_second() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - Doggy");
    }
    ...
    fn reset_clears_state_and_emits_grok() {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, "Doggy");
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("Doggy"), "title missing product 'Doggy': {t}");
    ...
    fn all_items_present_in_order() {
        ...
        assert_eq!(
            mgr.last_title,
            "Thinking - proj - grok-3 - workspace - Doggy"
        );
    }
