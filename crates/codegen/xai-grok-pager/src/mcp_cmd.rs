...
Examples:
  ...
  # Add to the project config (./.Doggy/config.toml) instead of ~/.Doggy/config.toml
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.Doggy/config.toml`, available in all your projects
    User,
    /// `./.Doggy/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Config to write to: user (~/.Doggy/config.toml) or project (./.Doggy/config.toml)
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    // ancestor .Doggy/config.toml, where it still resolves for sessions.
...
mod tests {
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        ...
        let project = PathBuf::from("/repo/.Doggy/config.toml");
