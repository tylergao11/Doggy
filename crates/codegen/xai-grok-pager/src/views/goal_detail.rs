    ...
    if goal.completion_phase.is_some() || !goal.completion_findings.is_empty() {
        ...
        if y < inner.y + inner.height {
            ...
            buf.set_line_safe(
                ...
                y,
                &Line::from(vec![
                    Span::styled("  Phase: ", Style::default().fg(theme.gray)),
                    Span::styled(phase_label, Style::default().fg(theme.accent_plan)),
                ]),
                w,
            );
            y += 1;
        }
        if goal.completion_findings.is_empty() {
            if y < inner.y + inner.height {
                buf.set_line_safe(
                    x,
                    y,
            ...
            for (i, f) in goal.completion_findings.iter().enumerate() {
                if y >= inner.y + inner.height {
                    break;
                }
                let sev = f
                    ...
                    .map(|s| format!("[{s}] "))
                    .unwrap_or_default();
                let line = format!("  {}. {sev}{}", i + 1, f.message);
                buf.set_line_safe(
                    x,
                    y,
                    &Line::from(Span::styled(line, Style::default().fg(theme.accent_error))),
                    w,
                );
                y += 1;
            }
