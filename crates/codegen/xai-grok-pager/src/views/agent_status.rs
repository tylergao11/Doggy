...
/// disagree.
///
/// Priority: Doggy `completion_phase` (audit gate) → legacy
/// `verifying_completion` / `planning` overlays → steady planner phase.
pub fn active_phase_label(goal: &GoalDisplayState) -> String {
    if let Some(cp) = goal.completion_phase {
        let base = cp.label().to_string();
        if cp == crate::app::agent::GoalCompletionPhase::Fixing
            && !goal.completion_findings.is_empty()
        {
            return format!("{base} ({})", goal.completion_findings.len());
        }
        return base;
    }
