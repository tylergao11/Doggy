...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    ...
    assert_eq!(
        ...
        "Plan product mode is deleted — Plan stages as Auto (no deferred SessionMode)",
    );
...
/// `/plan` on the dashboard stages Goal (Plan product mode removed), and
/// toggles back to Auto on a second press.
#[serial_test::serial(GROK_AGENT_DASHBOARD)]
#[test]
fn dashboard_slash_plan_toggles_pending_plan_mode() {
    ...
    assert_eq!(
        ...
        "first /plan must stage Goal for the next agent"
    );
    let _ = dispatch_dashboard_dispatch_slash(&mut app, "/plan".into());
    assert_eq!(
        app.dashboard.as_ref().unwrap().pending_mode,
        DashboardDispatchMode::Auto,
        "second /plan must toggle back to Auto"
    );
}
...
fn dashboard_plan_description_transforms_snapshot_and_chip_ranges() {
    ...
    assert_eq!(
        ...
        Some(xai_grok_tools::types::SessionMode::Goal),
        "`/plan <description>` stages Goal for the next agent"
    );
...
/// A staged model + Plan dispatch mode are applied to the agent spawned by
/// the next dispatch: the model id threads into `CreateSession`, the effort
/// is stashed as a deferred switch. Plan product mode is deleted — Plan
/// stages as Auto (no deferred SessionMode / plan pending).
#[serial_test::serial(GROK_AGENT_DASHBOARD)]
#[test]
fn dashboard_dispatch_applies_pending_model_and_plan() {
    ...
    assert_eq!(
        agent.deferred_session_mode, None,
        "Plan stages as Auto — no deferred SessionMode"
    );
    assert_eq!(agent.plan_mode_pending, Some(false));
}
...
/// path. Plan product mode is deleted — Plan stages as Auto.
#[serial_test::serial(GROK_AGENT_DASHBOARD)]
#[test]
...
/// The deferred Goal `SessionMode` is emitted (and cleared) once the
...
fn dashboard_deferred_plan_mode_applied_on_session_created() {
    ...
    assert!(
        ...
        "SessionCreated must emit SetSessionMode for the deferred Goal mode"
    );
}
