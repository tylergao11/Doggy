...
fn cycle_mode_pre_session_blocked_by_policy_pin() {
    ...
    assert!(
        ...
        "pre-session Goal→Auto must not stage yolo under the pin"
    );
...
/// Pre-session cycle from Goal with STALE `yolo_mode = true` (e.g. client
/// state restored from before the pin landed): clears yolo under the pin
...
fn cycle_mode_pre_session_clears_stale_yolo_under_pin() {
    ...
    assert!(
        !agent.session.is_yolo(),
        "stale pre-session yolo must be cleared under the pin"
    );
    ...
    assert_eq!(
        ...
        Some("auto"),
        "resident cycle always persists Auto permission under the two-posture ring"
    );
}
...
/// on repeated presses. Ring is Auto ↔ Goal.
#[test]
fn dispatch_cycle_mode_pre_session_cycles_locally_and_creates_session() {
    ...
    let agent = &app.agents[&AgentId(0)];
    assert_eq!(
        agent.goal_mode_pending,
        Some(true),
        "pre-session Auto → Goal must set optimistic pending"
    );
    assert_eq!(
        agent.deferred_session_mode,
        Some(xai_grok_tools::types::SessionMode::Goal),
        "Goal must be deferred to SessionCreated"
    );
    assert!(
        effects
            .iter()
    ...
    assert!(
        ...
        "pre-session Goal → Auto must persist the displayed mode, got {effects:?}"
    );
    let _ = dispatch(Action::CycleMode, &mut app);
    let agent = &app.agents[&AgentId(0)];
