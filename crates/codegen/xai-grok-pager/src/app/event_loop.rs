...
pub(crate) async fn run(
    ...
    // Apply initial config (may come from existing ~/.Doggy/pager.toml).
    ...
    loop {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.Doggy/config.toml [ui]`
                // rather than `~/.Doggy/pager.toml`. The watcher only knows
