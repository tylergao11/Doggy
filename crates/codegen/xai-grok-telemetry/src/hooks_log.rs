...
//! file logger that writes to `~/.Doggy/logs/hooks.log`.
//!
//! ## When to use
//!
...
//!
//! ```bash
//! GROK_HOOKS_LOG=1 grok              # enable, write to ~/.Doggy/logs/hooks.log
//! GROK_HOOKS_LOG=/tmp/h.log grok     # write to custom path
//! GROK_HOOKS_LOG=0 grok              # explicitly disable
//! tail -f ~/.Doggy/logs/hooks.log     # watch in another terminal
//! ```
...
///
/// Writes to `~/.Doggy/logs/hooks.log` (or custom path via `GROK_HOOKS_LOG`).
