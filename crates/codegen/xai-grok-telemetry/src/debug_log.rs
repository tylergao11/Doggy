...
//!   firehose to `~/.Doggy/debug/<session_id>.txt` (one file per session), with a
...
pub(crate) enum DebugTarget {
    /// `GROK_DEBUG_LOG=1` → route per session into `<dir>` (`~/.Doggy/debug`).
...
/// per session into `~/.Doggy/debug`, an explicit path writes a single file.
///
...
/// `~/.Doggy/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
