//! Parse marketplace sources from `~/.Doggy/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// and `known_marketplaces.json` files under `~/.Doggy/` and `~/.claude/`.
...
/// instead of `~/.Doggy`/`~/.claude`. Each root is checked for
