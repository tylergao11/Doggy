...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        for proto in protos {
            // Unix can stream deps to /dev/stdout and discard the descriptor
            // via /dev/null. Windows has neither path — use temp files.
            let dep_tmp = if cfg!(windows) {
            ...
            if let Some(ref dep_path) = dep_tmp {
                command.arg(format!(
                    "--dependency_out={}",
            ...
            if !output.status.success() {
                ...
                if let Some(ref p) = dep_tmp {
                    let _ = fs::remove_file(p);
                }
                return Err(anyhow::anyhow!("protoc command failed"));
            }

            let output = if let Some(ref dep_path) = dep_tmp {
                let text = fs::read_to_string(dep_path).context("read protoc dependency_out")?;
            ...
            // Makefile dep format: `target: deps…`. On Windows `target` is an
            // absolute path containing `:`, so split on the first `: ` (colon
            // space), not the first bare `:`.
            let rem = first_line
                .split_once(": ")
                .map(|(_, rest)| rest)
                .or_else(|| {
                    // Unix legacy `/dev/null:dep` with no space after colon.
                    first_line
                        .strip_prefix("/dev/null:")
                        .or_else(|| first_line.split_once(':').map(|(_, r)| r))
                })
                .with_context(|| {
                    format!("protoc dependency_out missing target separator: {output:?}")
                })?;
